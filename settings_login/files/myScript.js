...
var repo_key = "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";
...
var jsondeneme = ["kaya", "kaya2"];
console.log(jsondeneme[1]);



function controlRealKey(real_key, state) {
    ...
    fetch(url, { headers })
        ...
        .then(data => {
            ...
            setCookie("repokeycookie", JSON.stringify(jsonData), 365);
...
if(readCookie("repokeycookie") != readCookie("repokeycookie"))
	if(controlRealKey(real_key, 1) == 1) {
		menuState(1);
	} else {
		menuState(2);
	}
...
function loginPressed() {
	if(encrypted_key != null) {
		...
		if(key.length) {
			console.log(key);
			repo_key = decodeString(encrypted_key, key);
			console.log(repo_key);
			controlRealKey(repo_key);
			//repo_key = decodeString(repo_key, key);
			//console.log(repo_key);
		}
	}
	else
